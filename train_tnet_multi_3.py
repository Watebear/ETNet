...
from models.tnet_multi_3 import T_net_multi
from utils.util import *
from data.tensor_list import TensorList
...
def main(config):
    ...
    dsets['train'] = TensorList(
        path=config.train_path_prefix,
        tensor_path=config.train_tensor_prefix,
    ...
    dsets['test']  = TensorList(
        path=config.test_path_prefix,
        tensor_path=config.test_tensor_prefix,
        num_frames=config.num_frames,
    )
    ...
    for epoch in range(config.start_epoch, config.n_epochs+1):
        ...
        for i, batch in enumerate(dset_loaders['train']):
            ...
            total_loss = loss_au_softmax + 3 * loss_au_dice
            # backward
            total_loss.backward()
            optimizer.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--stepsize', type=int, default=4, help='epoch for decaying lr') ##################################################
    ...
    parser.add_argument('--num_frames', default=3, action='store_false',
                        help='use attention mask for Transformer (default: true)')
    ...
    parser.add_argument('--task_log_prefix', type=str, default='./exps/tnet_multi_tensor_512_123/')
    ...
    parser.add_argument('--train_tensor_prefix', type=str, default='./data/tensor_512/DISFA_combine_1_2/train')
    parser.add_argument('--test_path_prefix', type=str, default='./data/list/DISFA_part3')
    parser.add_argument('--test_tensor_prefix', type=str, default='./data/tensor_512/DISFA_combine_1_2/test')
