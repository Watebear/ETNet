...
from models.jaav2_multi.network import HMRegionLearning, AlignNet, LocalAttentionRefine, LocalAUNetv2, HLFeatExtractor, AUNet
from models.losses.losses import au_softmax_loss, au_dice_loss, landmark_loss
from models.jaav2_multi.t_net import T_net_multi

class JaaNetv2_multi(nn.Module):
    def __init__(self, config):
        super(JaaNetv2_multi, self).__init__()
        # misc
    ...
    def get_jaafeat(self, inputs):
        ...
        for i in range(5):
            ...
            concat_au_feat_list.append(concat_au_feat)
...
if __name__ == "__main__":
    ...
    net = JaaNetv2_multi(config).cuda()
    inp = torch.randn((10, 5, 3, 176, 176)).cuda()
    aus_output, local_aus_output, align_output = net(inp)

    #torch.save(net.state_dict(), './demo_layer5_dim512.pth')   # 2G
    #torch.save(net.state_dict(), './demo_layer2_dim512.pth')   # 896.9MB
    #torch.save(net.state_dict(), './demo_layer2_dim128.pth')   # 135.5MB
    #torch.save(net.state_dict(), './demo_layer1_dim128.pth')   # 111.8MB
    #torch.save(net.state_dict(), './demo_layer1_dim16.pth')    # 84.1MB
    #torch.save(net.state_dict(), './demo_layer2_dim16.pth')     # 84.5MB